...
import { Plus, Pencil, X, Check, Package, AlertCircle } from 'lucide-react'
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-[14px] mt-1">Manage your products</p>
        </div>
        <button
          onClick={() => setEditing({ ...emptyProduct })}
          className="flex items-center gap-2 px-4 py-2.5 bg-primary text-primary-foreground text-[12px] tracking-[0.15em] rounded-lg hover:bg-primary/90 transition-colors cursor-pointer"
        >
          ...
          <div key={product.id} className={`glass-card rounded-xl p-5 flex items-center gap-5 ${!product.available ? 'opacity-50' : ''}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-0.5">
                <h3 className="font-serif text-[16px] text-foreground">{product.name}</h3>
                {product.badge && (
                  <span className="px-2 py-0.5 bg-primary/20 text-primary text-[9px] tracking-[0.15em] rounded">
                    {product.badge}
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                {product.available ? <Check className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5" />}
              </button>
              <button
                onClick={() => setEditing({ ...product })}
                className="w-8 h-8 rounded-lg border border-border/50 flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-foreground/50 transition-colors cursor-pointer"
                title="Edit"
              >
              ...
              <button
                ...
                className="w-8 h-8 rounded-lg border border-border/50 flex items-center justify-center text-muted-foreground hover:text-red-400 hover:border-red-400/50 transition-colors cursor-pointer"
                title="Delete"
              >
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={() => setEditing(null)}>
          <div className="bg-card border border-border/30 rounded-xl p-6 w-full max-w-lg mx-4 space-y-4" onClick={e => e.stopPropagation()}>
            <h2 className="font-serif text-xl text-foreground">{editing.id ? 'Edit Product' : 'Add Product'}</h2>

            {error && (
              <div className="flex items-center gap-2 text-[12px] text-red-400 bg-red-400/10 rounded-lg px-4 py-3">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[11px] tracking-[0.1em] text-muted-foreground block mb-1">Name</label>
                <input
                  value={editing.name || ''}
              ...
              <div>
                <label className="text-[11px] tracking-[0.1em] text-muted-foreground block mb-1">Description</label>
                <textarea
                  value={editing.description || ''}
                  onChange={e => setEditing(f => ({ ...f, description: e.target.value }))}
                  className="w-full bg-transparent border border-border/50 rounded-lg px-4 py-2.5 text-[14px] text-foreground focus:border-primary outline-none min-h-[60px]"
                  placeholder="Product description"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] tracking-[0.1em] text-muted-foreground block mb-1">Price (£)</label>
                  <input
                    type="number"
                    step="0.01"
                    value={editing.price || ''}
                ...
                <div>
                  <label className="text-[11px] tracking-[0.1em] text-muted-foreground block mb-1">Unit</label>
                  <input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] tracking-[0.1em] text-muted-foreground block mb-1">Category</label>
                  <select
                ...
                <div>
                  <label className="text-[11px] tracking-[0.1em] text-muted-foreground block mb-1">Badge</label>
                  <input
                    value={editing.badge || ''}
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 px-4 py-3 border border-border/50 rounded-lg text-[12px] tracking-[0.15em] text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
              >
                CANCEL
              </button>
              <button
                onClick={save}
                disabled={saving}
                className="flex-1 px-4 py-3 bg-primary text-primary-foreground rounded-lg text-[12px] tracking-[0.15em] hover:bg-primary/90 transition-colors disabled:opacity-50 cursor-pointer"
              >
                {saving ? 'SAVING...' : 'SAVE'}
              </button>
            </div>
      ...
      {products.length === 0 && !loading && (
        <div className="text-center py-20 glass-card rounded-xl">
          ...
          <p className="text-[15px] text-muted-foreground">No products yet. Add your first product.</p>
        </div>
      )}
    </div>
  )
